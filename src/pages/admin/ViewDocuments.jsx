...
try {
    ...
    // Force cache bypass
    const { data } = await api.post(
        `/candocspro/get-file?storageAreaId=${doc.storageId}&guid=${doc.guid}&_=${Date.now()}`,
        {},
        { headers: { 'Cache-Control': 'no-cache' } }
    );

        let previewUrl = "";
        let finalMimeType = data.mimeType || doc.mime || "application/octet-stream";
        const isWordDoc = finalMimeType.toLowerCase().includes("wordprocessingml.document");
        // ADD THIS LINE for Excel files
        const isExcelDoc = finalMimeType.toLowerCase().includes("spreadsheetml.sheet");
        const isPowerPointDoc =
        finalMimeType.toLowerCase().includes("presentationml") || // PPTX
        finalMimeType.toLowerCase() === "application/vnd.ms-powerpoint"; // old PPT
  ...
  try {
    ...
    if (slideImages.length > 0) {
      const slidesHtml = slideImages.map(
        (url, idx) =>
           ...
           if (isWordDoc) {
                ...
                mammoth.convertToHtml({arrayBuffer: arrayBuffer})
                    ...
                    .catch(error => {
                        ...
                        setDocxHtml(`<p class="vd-doc-unsupported">Failed to convert Word document for preview. <a href="${previewUrl}" download>Download file</a>.</p>`);
                    });
            }
            ...
            try {
                ...
                let htmlTable = XLSX.utils.sheet_to_html(workbook.Sheets[firstSheetName]);
...
const updateDocument = async (guid) => {
  try {
    ...
    formData.append(
      "className",
      editedDetails.customClassName || selectedDoc.document?.customClassName || "testing.Stephen701"
    );
    formData.append("guid", guid);

    // Document section
    const documentPayload = {
      documentTitle: editedDetails.documentTitle || selectedDoc.document?.documentTitle,
      ...
      defaultSharedLocation:
        editedDetails.defaultSharedLocation || selectedDoc.document?.defaultSharedLocation,
    };
    formData.append(
      "document",
    ...
    const customDataPayload =
      editedDetails.customData && Object.keys(editedDetails.customData).length > 0
        ? editedDetails.customData
        : selectedDoc.customData || {};

    formData.append(
      "customData",
      new Blob([JSON.stringify(customDataPayload)], { type: "application/json" })
    );
    ...
    alertService.success("Success", data.message || "Document updated successfully!");
...
await fetchFile({
  ...
  mime: selectedDoc.mime
    ...
    alertService.error("Error", error.response?.data?.message || "Failed to update document.");
  } finally {
    setPreviewLoading(false);
  }
};
    ...
    return (
        ...
        <div className="vd-form-group">
            ...
            disabled={filters.length === 1 || index === filters.length - 1} 
 ...
 <Button
  ...
  onClick={() => {
    if (isEditing) {
      ...
      setIsEditing(true);
      setEditedDetails({
        documentTitle: selectedDoc.document?.documentTitle || "",
        version: selectedDoc.document?.version || "",
        mimeType: selectedDoc.document?.mimeType || selectedDoc.mime || "",
        customClassName: selectedDoc.document?.customClassName || "",
      });
    }
  }}
  style={{
...
<div className="vd-preview-section">
  ...
  {selectedDoc?.preview || docxHtml ? (
   ...
   // ✅ Image & GIF files
selectedDoc.mime?.toLowerCase().startsWith("image") ? (
  selectedDoc.mime?.toLowerCase().includes("gif") ? (
    <div className="vd-gif-preview">
      <img
        src={selectedDoc.preview}
        alt={selectedDoc.document?.documentName || "GIF Preview"}
      ...
      />
      <p style={{ textAlign: "center", color: "#999", marginTop: "5px" }}>
...
// ✅ HTML file preview
(selectedDoc.mime?.toLowerCase().includes("text/html") ||
 selectedDoc.fileName?.toLowerCase().endsWith(".html") ||
 selectedDoc.fileName?.toLowerCase().endsWith(".htm")) &&
...
(selectedDoc.mime?.toLowerCase().includes("xml") ||
 ...
 selectedDoc.fileData ? (
  ...
  >
    {(() => {
      try {
        const byteArray = Uint8Array.from(atob(selectedDoc.fileData), c => c.charCodeAt(0));
        ...
        const formattedXml = decoded
          ...
          .map((line, i, arr) => {
            ...
            if (i > 0) {
              const prev = arr[i - 1];
              if (prev.match(/<[^/!?][^>]*>$/) && !prev.match(/<\/.+>$/)) indent++;
    ...
    (selectedDoc.mime?.toLowerCase().includes("text/plain") ||
      selectedDoc.fileName?.toLowerCase().endsWith(".log") ||
      selectedDoc.mime?.toLowerCase() === "application/octet-stream") &&
    selectedDoc.fileData ? (
      <pre
        className="vd-doc-preview"
      ...
      >
        {(() => {
          try {
            const byteArray = Uint8Array.from(atob(selectedDoc.fileData), c =>
              c.charCodeAt(0)
            );
            const decoded = new TextDecoder("utf-8").decode(byteArray);
            return decoded;
    ...
    // ✅ RTF (.rtf) files
    (selectedDoc.fileName?.toLowerCase().endsWith(".rtf") ||
      selectedDoc.mime?.toLowerCase() === "application/rtf" ||
      selectedDoc.mime?.toLowerCase() === "text/rtf" ||
      selectedDoc.mime?.toLowerCase() === "application/msword") &&
    selectedDoc.fileData ? (
      <div
        className="vd-doc-preview"
        style={{ maxHeight: "500px", overflow: "auto", whiteSpace: "pre-wrap" }}
    ...
    // ✅ CSV file
    selectedDoc.mime?.toLowerCase() === "text/csv" && selectedDoc.fileData ? (
      <div
    ...
    // ✅ MP4 video preview
(selectedDoc.mime?.toLowerCase().includes("video/mp4") ||
 selectedDoc.fileName?.toLowerCase().endsWith(".mp4")) &&
 selectedDoc.preview ? (
  <div className="vd-video-preview" style={{ textAlign: "center" }}>
    <video
      src={selectedDoc.preview}
      controls
      style={{
    ...
    <p style={{ color: "#888", marginTop: "5px" }}>🎥 MP4 Video Preview</p>
  </div>
) :

// ✅ MP3 audio preview
(selectedDoc.mime?.toLowerCase().includes("audio/mpeg") ||
 selectedDoc.mime?.toLowerCase().includes("audio/mp3") ||
 selectedDoc.fileName?.toLowerCase().endsWith(".mp3")) &&
 selectedDoc.preview ? (
  <div className="vd-audio-preview" style={{ textAlign: "center" }}>
    <audio
      controls
      src={selectedDoc.preview}
      style={{
    ...
    <p style={{ color: "#888", marginTop: "5px" }}>🎧 MP3 Audio Preview</p>
  </div>
) :
...
<div className="vd-details-section">
  ...
  <table className="vd-details-table">
    <tbody>
        ...
        <td>
          {isEditing ? (
            <input
              ...
              onChange={(e) =>
                setEditedDetails({ ...editedDetails, documentTitle: e.target.value })
              }
              className="vd-form-input"
            />
          ) : (
      ...
      <tr>
        <td>Upload Date</td>
        <td>{selectedDoc.createdAt ? new Date(selectedDoc.createdAt).toLocaleString() : "N/A"}</td>
      </tr>
      <tr>
        <td>Version</td>
      ...
      <tr>
        <td>Mime Type</td>
        <td>{selectedDoc.document?.mimeType || selectedDoc.mime}</td>
      </tr>
      {/* <tr>
      ...
      <tr>
        ...
        <td>{selectedDoc.document?.customClassName?.split('.').pop() || "N/A"}</td>
      </tr>
      {isEditing && (
        <tr>
          <td>Replace File</td>
          <td>
            <input
              type="file"
              onChange={(e) =>
                setEditedDetails({ ...editedDetails, file: e.target.files[0] })
              }
              className="vd-form-input"
            />
          </td>
                ...
                {isEditing ? (
                  key === "storageAreaId" ? (
                    ...
                    <input
                      type="text"
                      value={
                        editedDetails.customData?.[key] !== undefined
